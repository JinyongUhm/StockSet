...
#If 0 Then
    ...
    Private _PrecalForAverage10kMinutes As Long
...
#If MOVING_AVERAGE_DIFFERENCE Then
    Public MainDecisionMakerCenter As New List(Of List(Of c05G_MovingAverageDifference))
...
#End If
    'Public TestDecisionMakerCenter As New List(Of List(Of c05F_FlexiblePatternChecker))
...
#If MOVING_AVERAGE_DIFFERENCE Then
    ...
    Public Shared MAX_NUMBER_OF_CANDLE As Integer = 6810
...
#If MOVING_AVERAGE_DIFFERENCE Then
            MainDecisionMakerCenter.Add(New List(Of c05G_MovingAverageDifference))
#Else
            MainDecisionMakerCenter.Add(New List(Of c05E_PatternChecker))
#End If
        Next
...
#End If
        ...
        For index As Integer = 0 To TEST_NUMBER_OF_DECIDERS - 1
            TestDecisionMakerCenter.Add(New List(Of c05F_FlexiblePatternChecker))
        Next
#End If
...
#If MOVING_AVERAGE_DIFFERENCE Then
                main_decision_maker_list.Add(New c05G_MovingAverageDifference(Me, CandleStartTime + TimeSpan.FromSeconds(RecordList.Count * 5), index_decision_center, 0))
#Else
...
#End If
        ...
        Dim test_decision_maker_list As List(Of c05F_FlexiblePatternChecker)
        For index_decision_center As Integer = 0 To TEST_NUMBER_OF_DECIDERS - 1
            ...
            If Not monitoring_decision_maker_exist AndAlso Not NoMoreDecisionMaker Then
                ...
                test_decision_maker_list.Add(New c05F_FlexiblePatternChecker(Me, StartTime, index_decision_center, 2))
            End If
...
#End If
    ...
    Private Sub _T1101_ReceiveData(ByVal szTrCode As String) Handles _T1101.ReceiveData
        Dim post_thread As Threading.Thread = New Threading.Thread(AddressOf _T1100_ReceiveData_PostThread)
        post_thread.IsBackground = True
        post_thread.Start()
    End Sub

    Private Sub _T1100_ReceiveData_PostThread()
        ...
        MessageLogging("A" & CodeForRequest & " : 매도1호가 " & _LastCallPrices.SelPrice1.ToString) ' & "   " & _LastCallPrices.SelAmount1.ToString)
        MessageLogging("A" & CodeForRequest & " : 매수1호가 " & _LastCallPrices.BuyPrice1.ToString) ' & "   " & _LastCallPrices.BuyAmount1.ToString)
        ...
        If call_prices.BuyAmount4 = 0 AndAlso call_prices.BuyAmount5 = 0 AndAlso call_prices.SelAmount4 = 0 AndAlso call_prices.SelAmount5 = 0 Then
            ...
            MessageLogging(Code & " : 호가를 받아보니 VI 상태네.")
        Else
            local_vi = False
    ...
    Public Sub SymbolTask()
            ...
            For index1 As Integer = 0 To SubDecisionMakerCenter.Count - 1
                For index2 As Integer = 0 To SubDecisionMakerCenter(index1).Count - 1
                    If SubDecisionMakerCenter(index1)(index2)._CurrentPhase = c050_DecisionMaker.SearchPhase.WAIT_EXIT_TIME Then
                        ...
                        If IsGoodTimeToBuy(Now) AndAlso (Not Caution) AndAlso (Not Supervision) AndAlso (Not SubDecisionMakerCenter(index1)(index2).YieldForHighLevel) Then    'SubDecisionMakerCenter(index1)(index2).EnterTime 에서 now로 바뀌었다. enter time이 안 좋은 시간대에 있어도 그 시간대 벗어나면 산다.
        ...
        For index1 As Integer = 0 To TestDecisionMakerCenter.Count - 1
            For index2 As Integer = 0 To TestDecisionMakerCenter(index1).Count - 1
                If TestDecisionMakerCenter(index1)(index2)._CurrentPhase = c050_DecisionMaker.SearchPhase.WAIT_EXIT_TIME Then
                    If TestDecisionMakerCenter(index1)(index2).VI_CheckStatus = c050_DecisionMaker.VI_CheckStatusType.WAIT_UNLOCK Then
                        'VI가 처음 풀린 것이다. EnterPrice 업데이트 해야 된다.
    ...
    Public Sub _T1102_ReceiveData_PostThread()
        ...
        Catch ex As Exception
            evidan_rate = 100
        End Try
        MessageLogging("A" & CodeForRequest & " :" & "증거금률 수신됨 : " & EvidanRate)
    ...
    Private Sub _HX_ReceiveData(ByVal szTrCode As String) Handles _H1_.ReceiveRealData, _HA_.ReceiveRealData
        ...
        If MarketKind = MARKET_KIND.MK_KOSPI Then
            ...
            call_prices.SelPrice1 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "offerho1"))
            call_prices.SelPrice2 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "offerho2"))
            call_prices.SelPrice3 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "offerho3"))
            call_prices.SelPrice4 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "offerho4"))
            call_prices.SelPrice5 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "offerho5"))
            call_prices.SelPrice6 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "offerho6"))
            ...
            call_prices.SelAmount1 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "offerrem1"))
            call_prices.SelAmount2 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "offerrem2"))
            call_prices.SelAmount3 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "offerrem3"))
            call_prices.SelAmount4 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "offerrem4"))
            call_prices.SelAmount5 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "offerrem5"))
            ...
            call_prices.BuyPrice1 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "bidho1"))
            call_prices.BuyPrice2 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "bidho2"))
            call_prices.BuyPrice3 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "bidho3"))
            call_prices.BuyPrice4 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "bidho4"))
            call_prices.BuyPrice5 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "bidho5"))
            call_prices.BuyPrice6 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "bidho6"))
            ...
            call_prices.BuyAmount1 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "bidrem1"))
            call_prices.BuyAmount2 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "bidrem2"))
            call_prices.BuyAmount3 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "bidrem3"))
            call_prices.BuyAmount4 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "bidrem4"))
            call_prices.BuyAmount5 = Convert.ToUInt32(_H1_.GetFieldData("OutBlock", "bidrem5"))
        ...
        Else 'If MarketKind = MARKET_KIND.MK_KOSDAQ Then
            _PriceUpdateCount = _PriceUpdateCount + 1
            call_prices.SelPrice1 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "offerho1"))
            call_prices.SelPrice2 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "offerho2"))
            call_prices.SelPrice3 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "offerho3"))
            call_prices.SelPrice4 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "offerho4"))
            call_prices.SelPrice5 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "offerho5"))
            call_prices.SelPrice6 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "offerho6"))
            ...
            call_prices.SelAmount1 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "offerrem1"))
            call_prices.SelAmount2 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "offerrem2"))
            call_prices.SelAmount3 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "offerrem3"))
            call_prices.SelAmount4 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "offerrem4"))
            call_prices.SelAmount5 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "offerrem5"))
            ...
            call_prices.BuyPrice1 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "bidho1"))
            call_prices.BuyPrice2 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "bidho2"))
            call_prices.BuyPrice3 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "bidho3"))
            call_prices.BuyPrice4 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "bidho4"))
            call_prices.BuyPrice5 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "bidho5"))
            call_prices.BuyPrice6 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "bidho6"))
            ...
            call_prices.BuyAmount1 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "bidrem1"))
            call_prices.BuyAmount2 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "bidrem2"))
            call_prices.BuyAmount3 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "bidrem3"))
            call_prices.BuyAmount4 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "bidrem4"))
            call_prices.BuyAmount5 = Convert.ToUInt32(_HA_.GetFieldData("OutBlock", "bidrem5"))
        ...
        Else
            If local_vi = True Then
                local_vi = False
                MessageLogging(Code & " : VI가 풀린 것을 축하하자")
                'VI 가 풀린 것을 축하하자
                SymTree.SymbolTaskToQueue(Me)
            End If
        End If
...
#End If
    ...
    Public Sub SetNewData() 'ByVal price As UInt32, ByVal amount As UInt64)
        ...
        If StartTime = [DateTime].MinValue Then
            ...
            StartTime = Now
        End If
        'If MinuteCandleSeries.Count = 0 AndAlso CandleStartTime = [DateTime].MinValue Then
        'CandleStartTime = Now
        'End If
...
#If 0 Then
        ...
        If _BuyAmountList.Count > 2 Then
            If buy_amount >= _BuyAmountList(0) Then
                BuyAveAmount = (buy_amount - _BuyAmountList(0)) / 3
            Else
                BuyAveAmount = 0
            End If
            If sel_amount >= _SelAmountList(0) Then
                SelAveAmount = (sel_amount - _SelAmountList(0)) / 3
            Else
                SelAveAmount = 0
            End If
...
#End If
        ...
        Dim test_decision_maker_list As List(Of c05F_FlexiblePatternChecker)
        Dim test_decision_maker As c05F_FlexiblePatternChecker
...
#End If
            ...
            For index As Integer = 0 To test_decision_maker_list.Count - 1
                ...
                If test_decision_maker.CurrentPhase = c050_DecisionMaker.SearchPhase.WAIT_FALLING OrElse test_decision_maker.CurrentPhase = c050_DecisionMaker.SearchPhase.UNWANTED_RISING_DETECTED OrElse
                    ...
                    (test_decision_maker.CurrentPhase = c050_DecisionMaker.SearchPhase.DONE And Not test_decision_maker.IsDone) Then
                    '위의 윗줄은 pre threshold 추가되면서 추가되었고, 위의 마지막 조건은 2시 48분 이후로 Decision maker 객체가 계속 증가하는 것을 방지하기 위해 추가되었다.
                    monitoring_decision_maker_exist = True
                    Exit For
                End If
            ...
            If Not monitoring_decision_maker_exist AndAlso Not NoMoreDecisionMaker Then
                '가격 모니터링 중인 decision maker가 한 개도 없다면 하나 만들어 리스트에 붙인다
                test_decision_maker_list.Add(New c05F_FlexiblePatternChecker(Me, StartTime + TimeSpan.FromSeconds((RecordList.Count - 1) * 5), index_decision_center, 2))
            End If
        Next
        SafeLeaveTrace(OneKey, 32)      'Leave critical zone, Thread간 공유문제 발생예방------------------------------------┘
#End If
...
#End If
        ...
        If MinuteCandleSeries.Count > MAX_NUMBER_OF_CANDLE Then
            ...
            CandleStartTime = MinuteCandleSeries(0).CandleTime
    ...
    Public Sub PastCandlesLoaded()
        ...
        'BaseFromPastCandles = MinuteCandleSeries.Count
...
#End If
    ...
    'Get Average10kMinutes
    Public Function GetAverage10kMinutes(ByVal new_price As UInt32) As Single
        If new_price = 0 Then
            'MinuteCandleSeries에 저장되어 있는 10k개의 price들을 평균냄
            If (MinuteCandleSeries.Count >= 10000) AndAlso DiscontinuousPoint - 1 < MinuteCandleSeries.Count - 10000 Then
                Dim new_sum_of_10k_prices As Single = _PrecalForAverage10kMinutes + MinuteCandleSeries(MinuteCandleSeries.Count - 10000).Close
                Return new_sum_of_10k_prices / 10000
            Else
                Return -1
            End If
        Else
            'MinuteCandleSeries에 저장되어 있는 10k개의 price + new_price 의 평균
            If MinuteCandleSeries.Count >= 10000 - 1 AndAlso DiscontinuousPoint < MinuteCandleSeries.Count - 10000 Then
                Dim new_sum_of_10k_prices As Single = _PrecalForAverage10kMinutes + new_price
                Return new_sum_of_10k_prices / 10000
            Else
                Return -1
            End If
    ...
    'Update Precal of Average10kMinutes
    Public Sub UpdateAverage10kMinutesPrecal()
        'Precal은 지난 10k개 종가의 합이 유지된다.
        Dim new_price = MinuteCandleSeries.Last.Close

        _PrecalForAverage10kMinutes += new_price
        If MinuteCandleSeries.Count >= 10000 Then
            _PrecalForAverage10kMinutes -= MinuteCandleSeries(MinuteCandleSeries.Count - 10000).Close
        End If
    End Sub
    ...
    Public Function MinuteCandleNewData(a_record As SymbolRecord) As Integer
        ...
        If IsGrowingCandleExist Then
            If (GrowingCandle.CandleTime.TimeOfDay = TimeSpan.FromHours(current_time.Hour) + TimeSpan.FromMinutes(current_time.Minute + 1)) OrElse IsTimeToKeepLastCandle(current_time) Then  '만들고 있는 캔들은 분 단위가 바뀔 때 기준으로 한다. 예를 들면 9시에 시작한 캔들은 9시 1분 캔들에 반영된다.
                ...
                If MinuteCandleSeries.Count > 0 Then
                    GrowingCandle.AccumAmount = MinuteCandleSeries.Last.AccumAmount + GrowingCandle.Amount
                Else
...
#If 0 Then
                If MinuteCandleSeries.Count >= 4 Then
                    ...
                    'Average5Minutes 업데이트
                    GrowingCandle.Average5Minutes = (GrowingCandle.Close + MinuteCandleSeries(MinuteCandleSeries.Count - 4).Close + MinuteCandleSeries(MinuteCandleSeries.Count - 3).Close + MinuteCandleSeries(MinuteCandleSeries.Count - 2).Close + MinuteCandleSeries(MinuteCandleSeries.Count - 1).Close) / 5
...
#End If
                If MinuteCandleSeries.Count > 0 Then
                    ...
                    End If
                    If growing_candle_low / MinuteCandleSeries.Last.Close < 0.7 OrElse growing_candle_high / MinuteCandleSeries.Last.Close > 1.3 Then
                ...
                MinuteCandleSeries.Add(GrowingCandle)
                If MinuteCandleSeries.Count > MAX_NUMBER_OF_CANDLE Then
                    MinuteCandleSeries.RemoveAt(0)
                    CandleStartTime = MinuteCandleSeries(0).CandleTime
                    If DiscontinuousPoint <> -1 Then
                ...
                new_candle.AccumAmount = MinuteCandleSeries.Last.AccumAmount + new_candle.Amount
                If new_candle.Amount > 0 Then
                ...
                Else
                    ...
                    If MinuteCandleSeries.Count > 0 Then
                        new_candle.Close = MinuteCandleSeries.Last.Close
                    Else
...
#End If
                    ...
                    new_candle.Amount = 0
                    AmountAtLastTime = a_record.CoreRecord.Amount
                    new_candle.AccumAmount = MinuteCandleSeries.Last.AccumAmount
                    new_candle.Open = a_record.CoreRecord.Price
                    new_candle.Close = new_candle.Open
                    new_candle.High = new_candle.Open
                    new_candle.Low = new_candle.Open
        ...
        End If
#If 0 Then
        If BaseFromPastCandles = MinuteCandleSeries.Count Then
        ...
        ElseIf minute_index + BaseFromPastCandles = MinuteCandleSeries.Count - 1 Then
            '190602: 이부분을 보완해야 한다. 앞으로 LoadPastCandles 들어가면 안 맞게 된다. 더구나 지금도 제 시간 지나서 좀 늦게 시작하면 안 맞는다.
            '190604: LoadPastCandles 완료후 MinuteCandleSeries의 count를 기록해두어 base로 사용해야 한다.
            ...
            End If
            NumberOfLastCandleUpdates += 1
        ElseIf minute_index + BaseFromPastCandles = MinuteCandleSeries.Count Then
            ...
            '새로운 candle 만들기
            Dim new_candle As CandleStructure
            new_candle.CandleTime = Now ' StartTime + TimeSpan.FromMinutes(MinuteCandleSeries.Count)
            ...
            'If a_record.Amount > MinuteCandleSeries.Last.AccumAmount Then
            'new_candle.Amount = a_record.Amount - MinuteCandleSeries.Last.AccumAmount
            'Else
            'new_candle.Amount = 0
            'End If
        ...
        For index_decision_center As Integer = MAIN_NUMBER_OF_DECIDERS - 1 To 0 Step -1
            ...
            If main_stock.Code <> "" Then
                ...
                Else
                    ...
                    WarningLogging(Now.Year & "-" & Now.Month & "-" & Now.Day & " Main 미연결 잔고: " & Name & " " & main_stock.Quantity & "개")
                End If
            Else
        ...
        For index_decision_center As Integer = SUB_NUMBER_OF_DECIDERS - 1 To 0 Step -1
            ...
            If sub_stock.Code <> "" Then
                ...
                If sub_decision_maker_list(0)._CurrentPhase = c050_DecisionMaker.SearchPhase.WAIT_EXIT_TIME Or sub_decision_maker_list(0)._CurrentPhase = c050_DecisionMaker.SearchPhase.DONE Then
                    ...
                    sub_decision_maker_list(0).SilentLevelVolume = sub_decision_maker_list(0).FallVolume * MainForm.SubAccountManager.SilentInvolvingAmountRate
                ...
                Else
                    MessageLogging("미연결 잔고: " & Code & " : " & sub_stock.Quantity & " 개")
                    WarningLogging(Now.Year & "-" & Now.Month & "-" & Now.Day & " Sub 미연결 잔고: " & Name & " " & sub_stock.Quantity & "개")
                End If
            Else
                '잘라온 게 없다. =>잔고 없는 것은 신경 쓸 필요 없다.
            End If
