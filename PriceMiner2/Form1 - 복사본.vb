...
#End If
        ...
        If _WhatToLoad = _EnumWhatToLoad.LOAD_SYMBOLCOLLECTION_PASTCANDLE Then
            'CybosLauncher를 실행한다.
            Process.Start("D:\Development\CybosLauncher\CybosLauncher\bin\Release\CybosLauncher.exe")
    ...
    Private Sub StockRestoring()
        ...
        If MainAccountManager.StoredStockList.Count > 0 Then
            ...
            For index As Integer = 0 To MainAccountManager.StoredStockList.Count - 1
                If MainAccountManager.StoredStockList(index).Code <> "A003620" OrElse MainAccountManager.StoredStockList(index).Code <> "A038340" OrElse MainAccountManager.StoredStockList(index).Code <> "A078590" Then
                    '20210305: 쌍용차도 제거
                    '20210331: UCI와 OQP도 제거
    ...
    Private Sub stm_PriceClock_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles stm_PriceClock.Elapsed
        ...
        If bt_ChartDataUpdate.Enabled = True Then
            ...
            If clock_div = 2 Then
                ...
                If ccc = 0 Then
                    request_result = MainAccountManager.RequestAccountInfo() ' 그냥 여기서 계좌정보 조회한다. 1초 후면 받겠지
                    If SubAccount Then
                        ccc = 1
                    Else
                        ccc = 2
                    ...
                    If request_result = -1 Then
                        Try
                            LoginProcess()
                        Catch ex As Exception
                            ErrorLogging("계좌정보 조회하다 실패해서 로그인시도하는데 익셉션")
                        End Try
                    End If
                ElseIf ccc = 1 Then
                    request_result = SubAccountManager.RequestAccountInfo() ' 그냥 여기서 계좌정보 조회한다. 1초 후면 받겠지
                    ccc = 2
                    If request_result = -1 Then
                    ...
                    End If
                Else 'if ccc = 2 Then
                    request_result = TestAccountManager.RequestAccountInfo() ' 그냥 여기서 계좌정보 조회한다. 1초 후면 받겠지
                    ccc = 0
                    If request_result = -1 Then
                        Try
                            LoginProcess()
        ...
        If IsLoadingDone Then
            If CpRestartStep = _EnumCpRestartStep.CP_RESTART_IDLE Then
                ...
                If is_stuck = 1 Then
                    ...
                    MessageLogging("CybosLauncher STUCK됨")
                    WarningLogging("CybosLauncher STUCK됨. 재시작 시도함.")
            ...
            If CpRestartStep = _EnumCpRestartStep.CP_RESTART_WAIT_CP_END Then
                ...
                If Not IsCpAlive Then
                    MessageLogging("CybosLauncher 종료됨")
            ...
            If CpRestartStep = _EnumCpRestartStep.CP_RESTART_CHECK_NETWORK Then
                ...
                Try
                    If My.Computer.Network.Ping("www.google.com") = True Then
                        ...
                        Process.Start("D:\Development\CybosLauncher\CybosLauncher\bin\Release\CybosLauncher.exe")
                        MessageLogging("CybosLauncher 재시작됨")
            ...
            If CpRestartStep = _EnumCpRestartStep.CP_RESTART_CHECK_CP_STUCK_RESOLVED Then
                ...
                If (is_stuck = 0) AndAlso is_real_data_mutex_open Then
                    MessageLogging("CybosLauncher 재가동완료")
            ...
            For retry_index As Integer = 0 To 10
                ...
                Catch ex As Exception
                    If LastExceptionErrorMesssage <> ex.Message Then
                        ...
                        ErrorLogging("Exception: 저장실패2" & ex.Message)
                    End If
        ...
        If WarningFileSaveTimeCount = 0 Then
            ...
            For retry_index As Integer = 0 To 10
                ...
                Catch ex As Exception
                    If LastExceptionErrorMesssage <> ex.Message Then
                        LastExceptionErrorMesssage = ex.Message
                        ErrorLogging("Exception: 저장실패3" & ex.Message)
                    End If
                    Threading.Thread.Sleep(200)
                End Try
            Next
...
#End If
    ...
    Private Sub MessageDisplay()
        ...
        Try
            If StoredMessagesMutex IsNot Nothing AndAlso StoredMessagesMutex.WaitOne(0) Then
...
#End If
    ...
    Private Sub SaveStocksForTomorrow()
        ...
        Dim quantity_attribute, ave_price_attribute As XmlAttribute
...
#End If
        ...
        For index As Integer = 0 To account_node.ChildNodes.Count - 1
            ...
            For decision_index As Integer = 0 To symbol_node.ChildNodes.Count - 1
                ...
                MessageLogging("Sub 잔고표시 " & a_stock.Code & ", " & a_stock.MA_Base & ", 갯수" & a_stock.Quantity & ", 평균단가 " & a_stock.AvePrice)
            Next
        Next
        '이 위로는 사실 StoreStockList 를 preemption으로 부터 보호할 필요가 없다. preemption 일어날 일이 없다.
    ...
    Private Sub LoginProcess()
        ...
        If xa_session.ConnectServer(svr_ip, svr_port) = False Then
            ...
            ErrorLogging(xa_session.GetErrorMessage(xa_session.GetLastError()))
            Exit Sub
        End If
        ' 로그인 한다.
        If xa_session.Login(user_id, user_password, user_cert, virtual_account_login, False) = False Then
            'MsgBox("로그인 전송 실패")
            ErrorLogging("로그인 전송 실패")
        Else
            '로그인 성공
            'MsgBox("로그인 전송 성공")
            MessageLogging("로그인 전송 성공")
            LoggedIn = True
        End If
    ...
    Private Sub XAReal_SC0_ReceiveRealData(ByVal szTrCode As String) Handles XAReal_SC0.ReceiveRealData
        ...
        'MessageLogging("SC0 " & account_number0)
        Dim account_number As String = XAReal_SC0.GetFieldData("OutBlock", "accno1") '계좌번호 추출
        MessageLogging("SC0_ " & account_number)
        Dim ordable_money_str As String = XAReal_SC0.GetFieldData("OutBlock", "ordablesubstamt")
        MessageLogging("SC0__ " & ordable_money_str)
        Dim order_number As String = XAReal_SC0.GetFieldData("OutBlock", "ordno")
        MessageLogging("SC0___ " & order_number)
        Dim tr_code As String = XAReal_SC0.GetFieldData("OutBlock", "trcode")
        MessageLogging("SC0____ " & tr_code)
        Dim short_code As String = XAReal_SC0.GetFieldData("OutBlock", "shtcode")
        MessageLogging("SC0_____ " & short_code)
        ...
        ElseIf account_number = TestAccountString Then
            ...
            simulate_thread.Start(parameters)
        Else
            ErrorLogging(account_number & "SC0 이건 무슨 계좌인가?")
        End If
    End Sub
    ...
    Private Sub XAReal_SC1_ReceiveRealData(ByVal szTrCode As String) Handles XAReal_SC1.ReceiveRealData
        ...
        MessageLogging("SC1 " & account_number0)
        Dim account_number As String = XAReal_SC1.GetFieldData("OutBlock", "accno1") '계좌번호 추출
        MessageLogging("SC1_ " & account_number)
        Dim order_number As Integer = Convert.ToInt32(XAReal_SC1.GetFieldData("OutBlock", "ordno")) '주문번호 추출
        MessageLogging("SC1__ " & order_number)
        Dim deal_price As Double = Convert.ToDouble(XAReal_SC1.GetFieldData("OutBlock", "ordavrexecprc"))   '주문평균체결가격 추출
        MessageLogging("SC1___ " & deal_price)
        Dim rest_amount As UInt32 = Convert.ToUInt32(XAReal_SC1.GetFieldData("OutBlock", "unercqty"))    '미체결수량(주문) 추출
        MessageLogging("SC1____ " & rest_amount)
        Dim deal_amount As UInt32 = Convert.ToUInt32(XAReal_SC1.GetFieldData("OutBlock", "execqty"))    '체결수량 추출
        MessageLogging("SC1_____ " & deal_amount)
        Dim buy_or_sel As Integer = Convert.ToUInt32(XAReal_SC1.GetFieldData("OutBlock", "bnstp"))      '매매구분 1:매수, 2: 매도
        MessageLogging("SC1______ " & buy_or_sel)
        Dim ordable_money_str As String = XAReal_SC1.GetFieldData("OutBlock", "ordablesubstamt")
        MessageLogging("SC1_______ " & ordable_money_str)
        Dim deposit As String = XAReal_SC1.GetFieldData("OutBlock", "deposit")
        MessageLogging("SC1________ " & deposit)
        Dim substamt As String = XAReal_SC1.GetFieldData("OutBlock", "substamt")
        MessageLogging("SC1_________ " & substamt)
        Dim csgnmnymgn As String = XAReal_SC1.GetFieldData("OutBlock", "csgnmnymgn")
        MessageLogging("SC1__________ " & csgnmnymgn)
        Dim csgnsubstmgn As String = XAReal_SC1.GetFieldData("OutBlock", "csgnsubstmgn")
        MessageLogging("SC1___________ " & csgnsubstmgn)
        Dim ordable_money As Long = Convert.ToInt64(ordable_money_str) '주문가능대용,ordablesubstamt
        ordable_money = Math.Max(ordable_money, 0)
...
#End If
        ...
        If account_number = MainAccountString Then
            ...
            parameters = {order_number, deal_price, rest_amount, deal_amount, ordable_money}
        ...
        ElseIf account_number = TestAccountString Then
            ...
            simulate_thread.Start(parameters)
        Else
            ErrorLogging(account_number & "SC1 이건 무슨 계좌인가?")
        End If
    End Sub
    ...
    Private Sub XAReal_SC3_ReceiveRealData(ByVal szTrCode As String) Handles XAReal_SC3.ReceiveRealData
        ...
        MessageLogging("SC3 " & account_number0)
        Dim account_number As String = XAReal_SC3.GetFieldData("OutBlock", "accno1") '계좌번호 추출
        MessageLogging("SC3_ " & account_number)
        Dim order_number As Integer = Convert.ToInt32(XAReal_SC3.GetFieldData("OutBlock", "ordno")) '주문번호 추출
        MessageLogging("SC3__ " & order_number)
        Dim original_order_number As Integer = Convert.ToInt32(XAReal_SC3.GetFieldData("OutBlock", "orgordno"))     '원주문번호추출
        MessageLogging("SC3___ " & original_order_number)
        Dim deal_price As Double = Convert.ToDouble(XAReal_SC3.GetFieldData("OutBlock", "ordavrexecprc"))   '주문평균체결가격 추출
        MessageLogging("SC3____ " & deal_price)
        Dim rest_amount As UInt32 = Convert.ToUInt32(XAReal_SC3.GetFieldData("OutBlock", "canccnfqty"))    '취소되어서 체결 안 된 수량 추출
        MessageLogging("SC3_____ " & rest_amount)
        Dim ordable_money_str As String = XAReal_SC3.GetFieldData("OutBlock", "ordablesubstamt")
        MessageLogging("SC3______ " & ordable_money_str)
        Dim ordable_money As Long = Convert.ToInt64(ordable_money_str)       '주문가능대용,ordablesubstamt
        ordable_money = Math.Max(ordable_money, 0)
        ...
        ElseIf account_number = SubAccountString Then
            ...
            simulate_thread.Start(parameters)
        ElseIf account_number = TestAccountString Then
            MessageLogging("Test" & " SC3 메시지 도착")
            ...
            simulate_thread.Start(parameters)
        Else
            ErrorLogging(account_number & "SC3 이건 무슨 계좌인가?")
        End If
    End Sub
    ...
    Private Sub XAReal_SC4_ReceiveRealData(ByVal szTrCode As String) Handles XAReal_SC4.ReceiveRealData
        ...
        Dim parameters() As Object
        If account_number = MainAccountString Then
            MessageLogging("Main" & " SC4 메시지 도착")
        ...
        ElseIf account_number = SubAccountString Then
            ...
            simulate_thread.Start(parameters)
        ElseIf account_number = TestAccountString Then
            MessageLogging("Test" & " SC4 메시지 도착")
            ...
            simulate_thread.Start(parameters)
        Else
            ErrorLogging(account_number & "SC4 이건 무슨 계좌인가?")
        End If
    ...
    Private Sub T8412ReceiveDataThread()
        ...
        If _T8412.Decompress("t8412OutBlock1") > 0 Then
            ...
            If count <= 3 Then
                MessageLogging("Data 갯수가 3이하야 아니 이럴 수도 있어?")
            End If
        Else
            count = 0
        End If
            ...
            If validity = True Then
                ...
                Catch ex As Exception
                    ...
                    Else
                        ...
                        If LastExceptionErrorMesssage <> ex.Message Then
                            LastExceptionErrorMesssage = ex.Message
                            ErrorLogging("Exception: 삽입실패" & ex.Message & ", " & date_str & " " & time_str)
                        End If
                    End If
                End Try
            Else
                '이건 삽입실패와 동일취급한다.
                _CandleChartInsertErrorCount += 1
                ErrorLogging("Invalid Price or Amount: " & open_str & ", " & high_str & ", " & low_str & ", " & close_str & ", " & amount_str)
                validity = True
            End If
            temp1 = Now.TimeOfDay
        ...
        For table_index As Integer = 0 To table_name_list.Count - 1
            ...
            Else
                ...
                Catch ex As Exception
                    ...
                    If LastExceptionErrorMesssage <> ex.Message Then
                        LastExceptionErrorMesssage = ex.Message
                        ErrorLogging("Exception: 생성1실패 " & ex.Message)
                    End If
...
#End If
            ...
            Do
                ...
                If RequestResult > 0 Then
                    ...
                    Do
                        ...
                        If a = 3000 Then
                            ...
                            ErrorLogging("ChartData " & table_index & ": " & SymbolCodeOnRequest & " no response")
                    ...
                    If (_CandleChartInsertErrorCount Mod 10000) > 0 Or (NoResponseNumber Mod 10000) > 0 Or (FailedSymbolNumber Mod 10000) > 0 Then
                        ...
                        Else
                            ...
                            ErrorLogging(table_index & "/" & table_name_list.Count & ", " & table_name_list(table_index) & "3번 retry 소용없어 skip한다")
                            local_retry_count = 0
                        End If
                    End If
                Else
                    ...
                    Else
                        ...
                        ErrorLogging("ChartData " & table_index & ": " & SymbolCodeOnRequest & " failed. " & RequestResult.ToString)
                        local_retry_count = 0
                        Exit Do
                    End If
                ...
                End If
                ProgressText3 = ", no response: " & NoResponseNumber & ", failed: " & FailedSymbolNumber & ", retry: " & _CandleChartRetryCount
                MessageLogging("ChartData " & table_index & ": " & SymbolCodeOnRequest & " 만들기 종료됨")
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        ...
        For index As Integer = 0 To SymbolList.Count - 1
            If SymbolList(index).MyOrderNumberList.Count <> 0 Then
                ...
                MessageLogging(SymbolList(index).Code & " : 이 심볼에 order data 가 조금 남아있어요. 오더 갯수는 이만큼 " & SymbolList(index).MyOrderNumberList.Count)
            End If
        Next

        MessageLogging("MyOrderList 검사결과 order data가 조금 남아있는 심볼 갯수가 이만큼 되네요. " & nok_symbol_count.ToString)
        MessageLogging("00600 주문객체 receive event 호출횟수 : " & OrderConfirmEventCount.ToString & ", Trace : " & OrderConfirmEventTracingKey.Var.ToString & ", " & OrderConfirmEventTracingKey.Time.ToString)
        MessageLogging("00600 주문객체 post thread 호출횟수 : " & OrderConfirmPostCount.ToString & ", Trace : " & OrderConfirmPostTracingKey.Var.ToString & ", " & OrderConfirmPostTracingKey.Time.ToString)
        MessageLogging("Main 계좌 sub confirm checker 등록수 : " & MainAccountManager.SubConfirmCheckerOperator.Count)
        MessageLogging("Sub 계좌 sub confirm checker 등록수 : " & SubAccountManager.SubConfirmCheckerOperator.Count)
        MessageLogging("Test 계좌 sub confirm checker 등록수 : " & TestAccountManager.SubConfirmCheckerOperator.Count)
    End Sub
#End If
    ...
    Private Sub XAReal_News_ReceiveRealData(szTrCode As String) Handles XAReal_News.ReceiveRealData
        ...
        For retry_index As Integer = 0 To 10
            ...
            Catch ex As Exception
                If LastExceptionErrorMesssage <> ex.Message Then
                    LastExceptionErrorMesssage = ex.Message
                    ErrorLogging("Exception: 저장실패4" & ex.Message)
                End If
                Threading.Thread.Sleep(200)
            End Try
        Next
